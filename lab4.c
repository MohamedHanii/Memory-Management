...
int main()
{
    ...
    strcat(outputFileName,"ch.txt\0");
...
void OPTIMALPolicy(int *pagesIn)
{
    ...
    while (pagesIn[i] != -1)
    {
        ...
        if(pagesIn[i]>=10) sprintf(output, "%d ", pagesIn[i]); // make the pagesIn[i] from int to string and put it in output
        else sprintf(output, "0%d ", pagesIn[i]); // make the pagesIn[i] from int to string and put it in output
        ...
        if (isExists(pagesIn[i], buffer) != -1)
        {
            ...
            for (j = 0; j < numberOfPages; j++)
            {
                ...
                */
                if (buffer[j] != -1)
                {
                    if(buffer[j]>=10)  sprintf(bufferstr, "%d ", buffer[j]); // make the pagesIn[i] from int to string and put it in output
                    else  sprintf(bufferstr, "0%d ", buffer[j]); // make the pagesIn[i] from int to string and put it in output
        ...
        if (isExists(pagesIn[i], buffer) != -1)
        {
            ...
            for (j = 0; j < numberOfPages; j++)
            {
                ...
                if (buffer[j] != -1)
                {
                    ...
                    // contact the the pageNo  with the buffer
                if(buffer[j]>=10)  
                    sprintf(bufferstr, "%d ", buffer[j]);             // make the pagesIn[i] from int to string and put it in output
                else  
                    sprintf(bufferstr, "0%d ", buffer[j]);          // make the pagesIn[i] from int to string and put it in output
                strcat(output, bufferstr);            
                }
            }
